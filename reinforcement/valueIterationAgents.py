...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
          ...
          for state in allStates:
            ...
            if bestAction != None:
              Qval = self.getQValue(state, bestAction)
              temp[state] = Qval
          self.values = temp
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for nextState, probability in self.mdp.getTransitionStatesAndProbs(state, action):
          update += probability * (self.mdp.getReward(state, action, nextState) 
    ...
    def computeActionFromValues(self, state):
        ...
        legalAction = self.mdp.getPossibleActions(state)
        maxVal = float("-inf")
        bestAction = None
...
class PrioritizedSweepingValueIterationAgent(AsynchronousValueIterationAgent):
        ...
        for i in range(self.iterations):
          ...
          if not self.mdp.isTerminal(s):
            ...
            for p in predecessors[s]:
              ...
              maxQval = self.getQValue(p, self.getAction(p))
              diff = abs(currentVal - maxQval)
              if diff > self.theta:
                pq.update(p, -diff)
    ...
    def setPriorityQueue(self):
      pq = util.PriorityQueue()
      for state in self.mdp.getStates():
        if not self.mdp.isTerminal(state):
          currentVal = self.getValue(state)
          maxQval = self.getQValue(state, self.getAction(state))
          diff = abs(currentVal - maxQval)
          pq.push(state, -diff)
      return pq
