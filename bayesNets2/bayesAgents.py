    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            for assignment in obsVarfactor.getAllPossibleAssignmentDicts():
                ...
                red = 0
                neither = 0
                if assignment[FOOD_HOUSE_VAR] == square:
                    red = PROB_FOOD_RED
                elif assignment[GHOST_HOUSE_VAR] == square:
                    red = PROB_GHOST_RED
                else:
                    neither = 1
                if assignment[obsVar] == RED_OBS_VAL:
                    ...
                    obsVarfactor.setProbability(assignment, 1 - red - neither)
                else:
                    obsVarfactor.setProbability(assignment, neither)
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    best = None
    maxProb = 0
    for assigment in query.getAllPossibleAssignmentDicts():
        newProb = query.getProbability(assigment)
        if newProb > maxProb:
            maxProb = newProb
            best = assigment
    return best
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        # left = inference.inferenceByVariableElimination(self.bayesNet, [TOP_LEFT_VAL], evidence, eliminationOrder)
        # right = inference.inferenceByVariableElimination(self.bayesNet, [TOP_RIGHT_VAL], evidence, eliminationOrder)
        ...
        for assignment in query.getAllPossibleAssignmentDicts():
            if assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL:
                left += query.getProbability(assignment)
            elif assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                right += query.getProbability(assignment)

        leftExpectedValue = (left * WON_GAME_REWARD) + (right * GHOST_COLLISION_REWARD)
        rightExpectedValue = (right * WON_GAME_REWARD) + (left * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        # maxVal= max(self.computeEnterValues(evidence, enterEliminationOrder))
        # print(maxVal)
        
        # pdb.set_trace()
        outcomes = self.getExplorationProbsAndOutcomes(evidence)
        for outcome in outcomes:
            # maxVal += max(self.computeEnterValues(outcome[1], enterEliminationOrder))
            update = evidence.copy()
            update.update(outcome[1])
            expectedValue += (outcome[0] * max(self.computeEnterValues(update, enterEliminationOrder)))
